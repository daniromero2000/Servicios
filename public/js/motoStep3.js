angular.module('appMotoStep3', ['moment-picker', 'ng-currency'])
.controller("motoStep3Ctrl", function($scope, $http) {
	$scope.quota = 0;
	$scope.numSolic = 0;
	$scope.leadInfo = {
		step: 3,
	...
	$scope.getDataStep3 = function(){
		...
		$http({
		  method: 'GET',
		  url: '/motos/solicitud/getDataMotoStep3/'+$scope.leadInfo.identificationNumber,
		...
		$http({
		  ...
		  data: $scope.comment,
			...
			if (response.data != false) {
				window.location="/motos/solicitud/thankYouPage";
			}
		}, function errorCallback(response) {
		    console.log(response);
		});
