...
angular.module('appQuotations', ['angucomplete-alt', 'flow', 'moment-picker', 'ng-currency', 'ngSanitize', 'ngTagsInput'])
    ...
    .controller('quotationsController', function ($scope, $http, $timeout) {
            ...
            if (($scope.items.sku != '') && (($scope.items.cod_proceso == 1) || ($scope.items.cod_proceso == 4))) {
                ...
                $http({
                    ...
                    if ($scope.items.sku == 'AV10' || $scope.items.sku == 'AV12' || $scope.items.sku == 'AV15') {
                        ...
                        $scope.items.price = Math.round((precio - (parseInt($scope.liquidator[key][2]) + parseInt($scope.liquidator[key][3].initial_fee))) * (parseInt(response.data.product[0].base_cost) / 100));
                        ...
                        if ($scope.items.sku == 'IVAV') {
                            ...
                            for (let i = 0; i < e.length; i++) {
                                if ((e[i].cod_proceso == 2) && ((e[i].sku == 'AV10') || (e[i].sku == 'AV12') || (e[i].sku == 'AV15'))) {
                                    $scope.items.price = Math.round(parseInt($scope.liquidator[key][0][i].price) * (parseInt(response.data.product[0].base_cost) / 100));
                                } else {
                                    $scope.items.price = 0
                                }
                            }
        ...
        $scope.createPlan = function (key) {
            if ($scope.liquidator[key][3].plan_id) {
                $scope.plans.forEach(e => {
                    if (e.sku == $scope.liquidator[key][3].plan_id) {
                        $scope.liquidator[key][8] = []
                        $scope.liquidator[key][3].plan = e.plan;
                        $scope.liquidator[key][8].push({ 'plan': $scope.liquidator[key][3].plan, 'plan_id': $scope.liquidator[key][3].plan_id })
                    }
                });
        ...
        $scope.addFee = function (key) {
            ...
            $scope.numberOfFees.forEach(e => {
                if (e.CUOTA == $scope.liquidator[key][3].term) {
                    factor = e.FACTOR;
                }
            });
            ...
            if ($scope.liquidator[key][3].term != null) {
                $scope.liquidator[key][3].value_fee = Math.round(((((precio - parseInt($scope.liquidator[key][2])) + (totalAval)) - (parseInt($scope.liquidator[key][3].initial_fee))) * factor))
                $scope.liquidator[key][7].push({ 'term': $scope.liquidator[key][3].term, 'value_fee': $scope.liquidator[key][3].value_fee, 'factor': factor });
            }
            ...
            $scope.liquidator[key][5].total = Math.round((parseInt($scope.liquidator[key][3].value_fee) * parseInt($scope.liquidator[key][3].term)) + parseInt
                ($scope.liquidator[key][3].initial_fee))
            ...
            $scope.liquidator[key][5].push({
                'total': $scope.liquidator[key][5].total, 'iva': $scope.liquidator[key][5].iva, 'subtotal': $scope.liquidator[key][5].subtotal
            });
            $scope.loader = false;
        ...
        $scope.sumDiscount = function (key) {
            ...
            switch ($scope.liquidator[key][3].plan_id) {
                case '1':
                    cuotaIni = 30000
                    break;
                case '3':
            ...
            $scope.liquidator[key][3].initial_fee = cuotaIni;
            $scope.liquidator[key][6].push({ 'initial_fee': cuotaIni });
                ...
                if (((e[i].sku == 'AV10') || (e[i].sku == 'AV12') || (e[i].sku == 'AV15'))) {
                    if ((e[i].cod_proceso == 2)) {
                        ...
                        $http({
                            ...
                            item.price = Math.round((precio - (parseInt($scope.liquidator[key][2]) + parseInt($scope.liquidator[key][3].initial_fee))) * (parseInt(response.data.product[0].base_cost) / 100))
                        ...
                        for (let h = 0; h < e.length; h++) {
                            if (e[h].sku == 'AV10' || e[h].sku == 'AV12' || e[h].sku == 'AV15') {
                                if (e[h].cod_proceso == 2) {
                                    item.price = Math.round(parseInt($scope.liquidator[key][0][h].price) * (parseInt(response.data.product[0].base_cost) / 100));
                                } else {
                                    item.price = 0
                                }
                            } else {
