...
namespace App\Http\Controllers\Admin\DigitalChannelLeadSlopes;
...
class DigitalChannelLeadSlopeController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $leadsOfMonth = $this->leadInterface->countLeadTotalSlopes($from, $to, $assessor);

        $skip = $this->toolsInterface->getSkip($request->input('skip'));
        $list = $this->leadInterface->listleadSlopes($skip * 30, $assessor);
        if (request()->has('q')) {
